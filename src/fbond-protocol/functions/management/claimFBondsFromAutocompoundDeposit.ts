...
type ClaimFbondsFromAutocompoundDeposit = (params: {
...
export const claimFbondsFromAutocompoundDeposit: ClaimFbondsFromAutocompoundDeposit = async ({ programId, connection, args, accounts, sendTxn }) => {
  ...
  const userBondsTokenAccount = await findAssociatedTokenAddress(accounts.userPubkey, accounts.fbondsTokenMint);
  ...
  instructions.push(
    await program.methods
      .claimFbondsFromAutocompoundDeposit(new BN(args.amountToClaim))
      .accounts({
        ...
        userBondsTokenAccount: userBondsTokenAccount,
